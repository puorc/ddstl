...
namespace test_min_element_comp
{
    ...
    {
        Iter i = ddstl::min_element(first, last, std::greater<int>());
        if (first != last)
        {
    ...
    {
        assert(first == ddstl::min_element(first, last, p));
    }
    ...
    {
#if TEST_STD_VER >= 14
        constexpr auto p = ddstl::min_element(il, il + 8, less());
        static_assert(*p == 1, "");
#endif
    }
}

TEST_CASE("test std::min_element with pred", "")
{
    using namespace test_min_element_comp;
