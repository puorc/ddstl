...
namespace test_set_symmetric_difference
{
    ...
    TEST_CONSTEXPR_CXX20 void test4()
    {
        ...
        const T b[] = {22, 32, 43, 42, 52};
        {
            ...
            OutIter end = ddstl::set_symmetric_difference(Iter1(a), Iter1(a + 4), Iter2(b), Iter2(b + 5), OutIter(result));
        ...
        }
        {
            T result[20] = {};
            T expected[] = {11, 22, 31, 42, 52};
            OutIter end = ddstl::set_symmetric_difference(Iter1(b), Iter1(b + 5), Iter2(a), Iter2(a + 4), OutIter(result));
            assert(std::lexicographical_compare(result, base(end), expected, expected + 5, T::less) == 0);
            for (const T *it = base(end); it != result + 20; ++it)
            {
                assert(it->value == 0);
...
TEST_CASE("test set_symmetric_differnce pass")
{
    using namespace test_set_symmetric_difference;
    test();
#if TEST_STD_VER > 17
    static_assert(test());
#endif
