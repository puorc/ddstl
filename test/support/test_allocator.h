...
public:
    ...
    template<class U>
    void construct(pointer p, U &&val) { ::new(static_cast<void *>(p)) T(std::forward<U>(val)); }
...
//    non_default_test_allocator() TEST_NOEXCEPT : data_(0) {++count;}
    ...
    template<class U>
    non_default_test_allocator(const non_default_test_allocator<U> &a) TEST_NOEXCEPT
            : data_(a.data_) { ++count; }
...
public:
    ...
    template<typename... Args>
    void construct(Tag_X *p, Args &&... args) { ::new((void *) p) Tag_X(Ctor_Tag{}, std::forward<Args>(args)...); }
...
bool
operator==(const TaggingAllocator<T> &, const TaggingAllocator<U> &) { return true; }

template<typename T, typename U>
bool
operator!=(const TaggingAllocator<T> &, const TaggingAllocator<U> &) { return false; }
...
class limited_allocator {
    template<class U, std::size_t UN> friend
    class limited_allocator;
