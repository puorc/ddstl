...
namespace test_equal_pred {
    ...
    TEST_CONSTEXPR bool test_constexpr() {
        ...
        return !ddstl::equal(std::begin(ia), std::end(ia), std::begin(ic)              , eq)
            && !ddstl::equal(std::begin(ia), std::end(ia), std::begin(ic), std::end(ic), eq)
            &&  ddstl::equal(std::begin(ib), std::end(ib), std::begin(ic)              , eq)
            && !ddstl::equal(std::begin(ib), std::end(ib), std::begin(ic), std::end(ic), eq)
    
            &&  ddstl::equal(II(std::begin(ib)), II(std::end(ib)), II(std::begin(ic))                  , eq)
            && !ddstl::equal(BI(std::begin(ib)), BI(std::end(ib)), BI(std::begin(ic)), BI(std::end(ic)), eq)
            ;
        }
#endif
...
TEST_CASE("test std::equal with pred pass.", "") {
    int ia[] = {0, 1, 2, 3, 4, 5};
