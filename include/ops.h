...
namespace ddstl
{
    ...
    template <typename BiPredicate>
    struct iter_bi_less_val_functor
    {
        BiPredicate &m_predicate;

        explicit iter_bi_less_val_functor(BiPredicate &pred) : m_predicate(pred) {}

        template <typename Iterator, typename Value>
        bool operator()(Iterator it, Value &v)
        {
            return bool(m_predicate(*it, v));
        }
    };

    struct iter_bi_less_val_const_functor
    {
        template <typename Iterator, typename Value>
        bool operator()(Iterator it, Value &v)
        {
