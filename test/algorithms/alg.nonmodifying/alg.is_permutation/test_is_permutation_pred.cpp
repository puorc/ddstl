...
namespace test_is_permutation_pred {
    ...
    constexpr bool test_constexpr() {
        ...
        int id[] = {1};
        std::equal_to<int> c{};
        return !ddstl::is_permutation(std::begin(ia), std::end(ia), std::begin(ib)              , c)
            && !ddstl::is_permutation(std::begin(ia), std::end(ia), std::begin(ib), std::end(ib), c)
            &&  ddstl::is_permutation(std::begin(ib), std::end(ib), std::begin(ic)              , c)
            &&  ddstl::is_permutation(std::begin(ib), std::end(ib), std::begin(ic), std::end(ic), c)
            && !ddstl::is_permutation(std::begin(ic), std::end(ic), std::begin(id), std::end(id), c)
            ;
        }
#endif
...
TEST_CASE("test is_permutation pred pass", "") {
    using namespace test_is_permutation_pred;
    {
        const int ia[] = {0};
        const int ib[] = {0};
        const unsigned sa = sizeof(ia) / sizeof(ia[0]);
        assert(ddstl::is_permutation(forward_iterator<const int *>(ia),
    ...
    }
    {
        ...
        comparison_count = 0;
        assert(ddstl::is_permutation(random_access_iterator<const int*>(ia),
    ...
    }
    {
        ...
        assert(ddstl::is_permutation(forward_iterator<const S *>(a),
