...
namespace test_distance {
    ...
    void check_distance(It first, It last, typename std::iterator_traits<It>::difference_type dist) {
        ...
        static_assert(std::is_same<decltype(ddstl::distance(first, last)), Difference>::value, "");
        assert(ddstl::distance(first, last) == dist);
    }
...
TEST_CASE("test std::distance pass", "") {
    using namespace test_distance;
    tests();
#if TEST_STD_VER >= 17
    static_assert(tests(), "");
#endif
